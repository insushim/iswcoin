...
export class DCAStrategy extends BaseStrategy {
  ...
  analyze(
    ...
    const dipThresholdPct = cfg["dipThresholdPct"] ?? 2;
    const dipMultiplier = cfg["dipMultiplier"] ?? 2;
    const rsiOversold = cfg["rsiOversold"] ?? 40;
    ...
    const recentHigh = Math.max(
      ...data.slice(-intervalCandles).map((d) => d.high),
    );
    const dipPct =
      recentHigh > 0
    ...
    const isScheduledBuy =
      candlesSinceLastBuy >= intervalCandles || holdingCandles === 0;
