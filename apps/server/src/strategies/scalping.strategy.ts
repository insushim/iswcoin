...
export class ScalpingStrategy extends BaseStrategy {
  ...
  analyze(
    ...
    const minRequired =
      Math.max(emaSlow, bbPeriod, atrPeriod * 2, volumeLookback, rsiPeriod) + 5;
    ...
    const currentADX =
      adxValues.length > 0 ? adxValues[adxValues.length - 1]!.adx : 0;

    // VWAP
    let aboveVWAP = true;
    let belowVWAP = true;
    if (vwapEnabled) {
      const vwapValues = indicatorsService.calculateVWAP(
        highs,
        lows,
        closes,
        volumes,
    ...
    // EMA 크로스
    const emaCrossUp =
      prevEmaFast <= prevEmaSlow && currentEmaFast > currentEmaSlow;
    const emaCrossDown =
      prevEmaFast >= prevEmaSlow && currentEmaFast < currentEmaSlow;
    ...
    const priceNearLowerBB =
      bbWidth > 0 && (currentPrice - currentBB.lower) / bbWidth < 0.15;
    const priceNearUpperBB =
      bbWidth > 0 && (currentBB.upper - currentPrice) / bbWidth < 0.15;
