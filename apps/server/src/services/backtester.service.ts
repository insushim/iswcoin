...
export class BacktesterService {
  ...
  async runBacktest(
    ...
    const splitIndex = Math.floor(
      ohlcvData.length * (config.walkForwardSplit || 0.7),
    );
    ...
    const fullResult = this.executeBacktest(
      ohlcvData,
      config,
      createAnalyzeFn(),
      slippage,
    ...
    if (inSampleData.length > 50 && outOfSampleData.length > 20) {
      const inSampleResult = this.executeBacktest(
        inSampleData,
        config,
        createAnalyzeFn(),
        slippage,
      ...
      const outOfSampleResult = this.executeBacktest(
        outOfSampleData,
        config,
        createAnalyzeFn(),
        slippage,
  ...
  private executeBacktest(
    ...
    const allocationPct =
      config.allocationPct ?? (positionMode === "accumulate" ? 20 : 100);
    ...
    for (let i = lookback; i < data.length; i++) {
      ...
      if (pendingSignal) {
        ...
        if (useDynamicSlippage) {
          ...
          const { currentATR, avgVolume } =
            slippageService.extractSlippageInputs(windowForSlippage);
        ...
        if (pendingSignal.action === "buy") {
          ...
          if (!position) {
            ...
            position.entries.length < maxEntries
          ) {
            // DCA 적립: 기존 롱에 추가 진입
            const allocCapital = capital * (allocationPct / 100);
            if (allocCapital > 0) {
              ...
              position.avgEntryPrice =
                position.totalCost / position.totalAmount;
              capital -= allocCapital;
            }
          }
      ...
      if (position) {
        if (position.side === "long") {
          currentEquity +=
            position.totalAmount * currentCandle.close * (1 - fee);
        } else {
          // 숏: 진입가 - 현재가 차이가 수익
          const shortPnl =
            (position.avgEntryPrice - currentCandle.close) *
      ...
      const enrichedConfig: Record<string, number> = {
      ...
      if (position) {
        ...
        if (position.side === "long") {
          enrichedConfig["_unrealizedPnlPct"] =
    ...
    // ===== 마지막 캔들의 pendingSignal 실행 (누락 방지) =====
    if (pendingSignal && data.length > lookback) {
      const lastCandle = data[data.length - 1]!;
      const openPrice = lastCandle.close; // 마지막 캔들은 종가로 체결

      let slippage = baseSlippage;
      if (useDynamicSlippage) {
        const windowForSlippage = data.slice(Math.max(0, data.length - 30));
        const { currentATR, avgVolume } =
          slippageService.extractSlippageInputs(windowForSlippage);
        const orderSize = capital * (allocationPct / 100);
        slippage = slippageService.calculateDynamicSlippage(
        ...
        if (!position) {
          ...
          if (allocCapital > 0) {
            ...
            position = {
              side: "long",
              entries: [
                { price: fillPrice, amount, timestamp: lastCandle.timestamp },
              ],
              totalAmount: amount,
              avgEntryPrice: fillPrice,
              totalCost: allocCapital,
              firstEntryTime: lastCandle.timestamp,
              firstEntryCandle: data.length - 1,
            };
            capital -= allocCapital;
          }
        }
  ...
  private calculateMetrics(
    ...
    const sharpeRatio =
      stdReturn > 0 ? (avgReturn / stdReturn) * annualizationFactor : 0;
    ...
    const downstdReturn =
      negativeReturns.length > 1
        ...
        : 0;
    const sortinoRatio =
      downstdReturn > 0 ? (avgReturn / downstdReturn) * annualizationFactor : 0;
    ...
    const annualizedReturn =
      durationDays > 0
        ? (Math.pow(finalCapital / initialCapital, 365 / durationDays) - 1) *
          100
        : 0;
    ...
    const calmarRatio =
      maxDrawdownPct > 0 ? annualizedReturn / maxDrawdownPct : 0;
  ...
  runMonteCarlo(
    ...
    for (let sim = 0; sim < simulations; sim++) {
      ...
      for (const trade of sampledTrades) {
        ...
        returns.push(
          prevCapital > 0 ? (capital - prevCapital) / prevCapital : 0,
        );
      }
      ...
      const stdReturn =
        returns.length > 1
          ...
          : 0;
      simSharpes.push(
        stdReturn > 0 ? (avgReturn / stdReturn) * Math.sqrt(trades.length) : 0,
      );
    }
    ...
    const result: MonteCarloResult = {
      ...
      worstMaxDrawdown:
        Math.round(simDrawdowns[simDrawdowns.length - 1]! * 100) / 100,
