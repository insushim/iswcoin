...
export class BotRunnerService {
  ...
  constructor() {
    ...
    this.realTrading = new RealTradingService(
      this.positionManager,
      this.orderCalculator,
    );
  }
  ...
  async startBotLoop(
    ...
    if (strategyType === "GRID" || strategyType === "MARTINGALE") {
      ...
      const savedStrategyState = rc?.strategyState as
      ...
      if (savedStrategyState) {
        if (strategyType === "GRID" && strategy instanceof GridStrategy) {
          ...
          strategy instanceof MartingaleStrategy
        ) {
          strategy.restoreState(savedStrategyState);
    ...
    let cachedExchange: ReturnType<typeof exchangeService.initExchange> | null =
      null;
    ...
    if (mode === "REAL") {
      const apiKeyRecord = await prisma.apiKey.findFirst({
        where: {
          userId,
          exchange: exchangeEnum as "BINANCE" | "UPBIT" | "BYBIT" | "BITHUMB",
          isActive: true,
        },
      });
      if (apiKeyRecord) {
      ...
      try {
        ...
        if (mode === "PAPER" && control.loopCount % PAPER_SAVE_INTERVAL === 0) {
          await this.paperTrading.savePaperState(botId).catch((err) => {
            logger.warn("Periodic paper state save failed", {
              botId,
              error: String(err),
            });
          });
        ...
        if (
          mode === "REAL" &&
          exchange &&
          control.loopCount % RECONCILE_INTERVAL === 0
        ) {
          await this.positionManager
            .reconcilePosition(botId, symbol, exchange, 0)
            .catch((err) =>
              logger.debug("Background task failed", { error: String(err) }),
            );
        }

        // OHLCV 데이터 가져오기 (Paper: 15분봉으로 더 빈번한 신호 생성)
        const timeframe = mode === "PAPER" ? "15m" : "1h";
        let ohlcvRaw: number[][] = [];
        if (mode === "PAPER") {
          // Paper 모드: 공개 API로 실시세 데이터 가져오기
          try {
            const publicExchange =
              exchangeService.getPublicExchange(exchangeName);
            const data = await exchangeService.getOHLCV(
              publicExchange,
              symbol,
              timeframe,
              200,
            ...
            logger.warn(
              "Paper mode: failed to fetch OHLCV via public exchange",
              { error: String(err) },
            );
        ...
        if (ohlcvData.length >= 15) {
          ...
          const atrValues = indicatorsService.calculateATR(
            highs,
            lows,
            closes,
            14,
        ...
        const enrichedConfig = this.orderCalculator.enrichConfigWithPosition(
          config,
          currentPosition,
          currentPrice,
        );
        ...
        let signal:
          | import("../../strategies/base.strategy.js").TradeSignal
          | null = null;
        if (strategyType === "FUNDING_ARB") {
          const { FundingArbStrategy } =
            await import("../../strategies/funding-arb.strategy.js");
          const fundingStrategy = strategy as InstanceType<
            typeof FundingArbStrategy
          >;
          try {
            ...
            if (
              fundingSignal.action === "ENTER_LONG_FUNDING" ||
              fundingSignal.action === "ENTER_SHORT_FUNDING"
            ) {
              signal = {
                action: "buy",
                ...
                stopLoss:
                  currentPrice *
                  (1 - (enrichedConfig["stopLossPercent"] ?? 2) / 100),
                metadata: {
            ...
            logger.warn("FundingArb analyzeWithFunding failed, falling back", {
              error: String(err),
            });
        ...
        if (mode === "PAPER" && (!signal || signal.action === "hold")) {
          ...
          if (paper) {
            ...
            const holdUnrealizedPnl =
              currentPosition && currentPosition.isOpen
                ? (currentPrice - currentPosition.entryPrice) *
                  currentPosition.amount
                : 0;
            const holdUnrealizedPnlPct =
              currentPosition &&
        ...
        if (signal && signal.action !== "hold") {
          ...
          if (
            ...
            logger.debug("Sell signal without open position, skipping", {
              botId,
            });
            return;
          }
          ...
          if (
            ...
            if (
              ...
              logger.debug(
                "Buy signal with existing position, skipping (non-DCA/ENSEMBLE)",
                { botId },
              );
              return;
            }
          ...
          if (mode === "PAPER") {
            ...
            await this.realTrading.executeRealTrade(
              botId,
              symbol,
              exchange,
              signal,
          ...
          if (strategyType === "GRID" || strategyType === "MARTINGALE") {
            try {
              const strategyState =
                strategyType === "GRID" && strategy instanceof GridStrategy
              ...
              if (strategyState) {
                ...
                const existingRc =
                  (existingBot?.riskConfig as Record<string, unknown>) ?? {};
              ...
              logger.warn("Failed to save strategy state", {
                botId,
  ...
  async stopBotLoop(botId: string): Promise<void> {
    ...
    if (control) {
      ...
      // Paper 모드 상태 저장
      await this.paperTrading.savePaperState(botId).catch((err) => {
        logger.error("Failed to save paper state on stop", {
          botId,
          error: String(err),
        });
      });
    ...
    for (const [botId, control] of this.state.activeBots) {
      ...
      savePromises.push(
        this.paperTrading.savePaperState(botId).catch((err) => {
          logger.error("Failed to save paper state during shutdown", {
            botId,
            error: String(err),
          });
        }),
  ...
  async recoverStuckBots(): Promise<void> {
    try {
      ...
      if (result.count > 0) {
        logger.warn(
          `Recovered ${result.count} stuck bot(s) from RUNNING to STOPPED`,
        );
      }
    } catch (err) {
